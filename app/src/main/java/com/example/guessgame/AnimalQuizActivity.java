...
public class AnimalQuizActivity extends AppCompatActivity {
    ...
    ImageButton aib;
    String option="";
    String[] imagename={"lion","tapir","proboscismonkey","redpanda","turtle","pinkfairyarmadillo","wolf","polarbear","sloth","zebra","okapi","bison","fossa","crocodile","snowleopard","dingo","cheetah","axolotl","beaver","slowloris","spermwhale","orangutan","tasmaniandevil","pygmymarmoset","spidercrab"};
    Set<Integer> s;
    Set<Integer> sb;
    ...
    public void onSubmit(View view) {
        ...
        if(option.equals(btn.getText().toString())){
            ...
            try{
                ...
                Scores res=realm.where(Scores.class).equalTo("userName",user).equalTo("userGame","Animals").findFirst();
                if(res==null){
                    Scores score=realm.createObject(Scores.class,System.currentTimeMillis());
                    score.setScore(5);
                    score.setUserName(user);
            ...
            if(wrongCount==3){
                ...
                Scores scObject=realm.where(Scores.class).equalTo("userName",playerName).equalTo("userGame","Animals").findFirst();
                if(scObject!=null){

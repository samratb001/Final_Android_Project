...
public class FlowerQuizActivity extends AppCompatActivity {
    ...
    String option="";
    String[] imagename={"lily","orchid","jasmine","begonia","rose","batplant","corpseflower","hookerslips","chrysanthemum","sunflower","petunia","parrotflower","dahlia","marigold","snowdrop","passionflower","proteapinwheel","lithopsweberi","pitcherplant","strelitzia","cherryblossom","daffodil","freesia","gerberas","anemone"};
    Set<Integer> s;
    Set<Integer> sb;
...
//        b4.setBackgroundColor(R.attr.colorButtonNormal);
        ...
        randomDouble = randomDouble * imglength;
    ...
    public void onSubmit(View view) {
        ...
        if(option.equals(btn.getText().toString())){
            ...
            try{
                if(realm.isEmpty()){
                    Scores score=realm.createObject(Scores.class,System.currentTimeMillis());
                    score.setUserName(user);
                    score.setUserGame("Flowers");
                    score.setScore(5);
                ...
                Scores res=realm.where(Scores.class).equalTo("userName",user).equalTo("userGame","Flowers").findFirst();
                if(res==null){
                    Scores score=realm.createObject(Scores.class,System.currentTimeMillis());
                    score.setScore(5);
                    score.setUserName(user);
            ...
            if(wrongCount==3){
                ...
                Scores scObject=realm.where(Scores.class).equalTo("userName",playerName).equalTo("userGame","Flowers").findFirst();
                if(scObject!=null){
